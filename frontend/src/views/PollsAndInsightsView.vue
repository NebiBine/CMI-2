    ...
    <div class="polls-and-insights-view">
        ...
        <div class="polls-container">
            <div v-for="poll in polls" :key="poll.id" class="EnPoll">
                ...
                <n-button @click="openModal">Participate</n-button>
                <n-modal v-model:show="showModal" preset="dialog" 
                title="Participate in the Poll"
                @positive-click="pollParticipationSubmit(poll.id)" 
                @negative-click="closeModal">
                    <!--IKONA MODALA-->
                    <template #icon>
                        <img src = "../assets/icons/poll_participate_icon.svg" alt="Poll Participate Icon" class="modal-icon"/>
                    </template>
                    ...
                    <div v-for="(question, questionIndex) in poll.questions" :key="questionIndex">
                        ...
                        <div v-else-if="question.type == 'input'" class="optionsWrapper">
                            <input type="text" :name="`poll-${poll.id}-q${questionIndex}`"
                        ...
                        <div v-else-if="question.type == 'checkbox'" class="optionsWrapper">
                            <template v-for="(option, optIndex) in question.options" :key="optIndex">
                                <label v-if="option" class="option-label">
                                    <input type="checkbox" :name="`poll-${poll.id}-q${questionIndex}-${optIndex}`"
                                        :value="option">
                                    {{ option }}
                                </label>
                            </template>
                    ...
                    <n-button type="primary" class="submitpollBtn" @click="pollParticipationSubmit(poll.id)">Submit your answers</n-button>
